...
ThingsBoardDevice::ThingsBoardDevice(const ThingsBoardConfig& config)
    ...
    mqtt_client_ = std::make_unique<MQTTClient>(server_uri, client_id, this);
...
bool ThingsBoardDevice::send_telemetry(int spot_id, double temperature) {
    ...
    if (result) {
        LOG_DEBUG("Telemetry message queued successfully");
    } else {
        LOG_ERROR("Failed to queue telemetry message");
    }
...
bool ThingsBoardDevice::send_telemetry(int spot_id, double temperature,
    ...
    if (result) {
        LOG_DEBUG("Timestamped telemetry message queued successfully");
    } else {
        LOG_ERROR("Failed to queue timestamped telemetry message");
    }
    
    return result;
}

const MQTTClientState& ThingsBoardDevice::get_connection_state() const {
...
void ThingsBoardDevice::on_message_delivered(const std::string& topic) {
    LOG_DEBUG("Telemetry message delivered to: " << topic);
}
...
void ThingsBoardDevice::on_connection_failure(const MQTTError& error) {
    LOG_ERROR("ThingsBoard connection failed: " << error.error_message 
             << " (code: " << error.error_code << ")");
    
    // Check for authentication failure
    if (error.error_code == MQTTASYNC_BAD_USERNAME_OR_PASSWORD || 
        error.error_code == MQTTASYNC_NOT_AUTHORIZED) {
        LOG_ERROR("Authentication failed. Check access token. Manual restart required.");
    }
}
